...
docker build -t raveekirann/multi-server:latest -t raveekirann/multi-server:$SHA -f ./server/Dockerfile ./server
...
kubectl set image deployments/client-deployment client=raveekirann/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=raveekirann/multi-worker:$SHA